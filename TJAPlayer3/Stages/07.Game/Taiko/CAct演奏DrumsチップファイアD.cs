        ...
        public virtual void Start( int nLane, E判定 judge, int player )
		{
            for (int j = 0; j < 3 * 4; j++)
            {
			    ...
			    //for( int n = 0; n < 1; n++ )
			    {
	    			...
	    			this.st状態[ j ].ct進行 = new CCounter( 0, 14, 18, TJAPlayer3.Timer );
                    ...
                    switch( nLane )
                    {
                        ...
                        case 0x1A:
                        case 0x1B:
                            this.st状態_大[ j ].ct進行 = new CCounter( 0, 20, 12, TJAPlayer3.Timer );
			...
			if( !base.b活性化してない )
			{
                    ...
                    if( this.st状態[ i ].b使用中 )
                    {
				        if( !this.st状態[ i ].ct進行.b停止中 )
				        {
					        ...
					        if( TJAPlayer3.Tx.Effects_Hit_Explosion != null && !TJAPlayer3.IsPerformingCalibration )
					        {
                                ...
                                int nX = ( TJAPlayer3.Skin.nScrollFieldX[ this.st状態[ i ].nPlayer ] ) - ( (TJAPlayer3.Tx.Effects_Hit_Explosion.sz画像サイズ.Width / 18 ) / 2 );
                ...
                for (int i = 0; i < 45; i++)
                {
                    ...
                    if (this.st大音符花火[i].b使用中)
                    {
                        ...
                        if(TJAPlayer3.Tx.Effects_Hit_FireWorks_1P != null && this.st大音符花火[ i ].nColor == 0 )
                        {
                            if( this.st大音符花火[ i ].n開始フレーム <= this.st大音符花火[ i ].ct進行.n現在の値 && this.st大音符花火[ i ].n終了フレーム > this.st大音符花火[ i ].ct進行.n現在の値 )
                            {
                                ...
                                TJAPlayer3.Tx.Effects_Hit_FireWorks_1P.t2D描画(TJAPlayer3.app.Device, (int)fX, (int)fY, new Rectangle( ( this.st大音符花火[i].ct進行.n現在の値 - this.st大音符花火[ i ].n開始フレーム ) * 192, 0, 192, 192 ) );
                            }
                        }
