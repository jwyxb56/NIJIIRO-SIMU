    ...
    //            CDTXMania.tテクスチャの解放( ref this.txPlayerNumber);
		...
		protected override void t進行描画_チップ_Taiko( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer )
        {
            ...
            if( pChip.b可視 )
            {
                if( !pChip.bHit || pChip.bShow )
				{
                    ...
                    {
                        x = (int)((((pChip.n発声時刻ms) - (pChip.n発声時刻ms - pChip.nノーツ移動開始時刻ms)) * pChip.dbBPM * pChip.dbSCROLL * (this.act譜面スクロール速度.db現在の譜面スクロール速度.Drums + 1.0)) / 502.8594);
                    }
                    else
                    {
                    ...
                    if( pChip.dbSCROLL_Y != 0.0 )
                    {
                        ...
                        if (TJAPlayer3.ConfigIni.eScrollMode == EScrollMode.Normal)
                            y += (int)(((pChip.n発声時刻ms - CSound管理.rc演奏用タイマ.n現在時刻) * pChip.dbBPM * pChip.dbSCROLL_Y * (this.act譜面スクロール速度.db現在の譜面スクロール速度.Drums + 1.0)) / 502.8594);
		...
		protected override void t進行描画_チップ_Taiko連打( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer )
        {
            ...
            if( pChip.b可視 )
            {
                if( pChip.nチャンネル番号 >= 0x15 && pChip.nチャンネル番号 <= 0x18 )
                {
                    ...
                    {
                        nノート座標 = (int)((((pChip.n発声時刻ms) - (pChip.n発声時刻ms - pChip.nノーツ移動開始時刻ms)) * pChip.dbBPM * pChip.dbSCROLL * (this.act譜面スクロール速度.db現在の譜面スクロール速度.Drums + 1.0)) / 502.8594);
                        nノート末端座標 = (int)(((pChip.nノーツ終了時刻ms - (pChip.n発声時刻ms - pChip.nノーツ移動開始時刻ms)) * pChip.dbBPM * pChip.dbSCROLL * (this.act譜面スクロール速度.db現在の譜面スクロール速度.Drums + 1.0)) / 502.8594);
                    }
                    else
                    {
                        nノート座標 = 0;
                ...
                if( pChip.nチャンネル番号 == 0x18 )
                {
                    ...
                    {
                        nノート座標 = (int)(((pChip.n発声時刻ms - (n先頭発声位置 - pChip.nノーツ移動開始時刻ms)) * pChip.dbBPM * pChip.dbSCROLL * (this.act譜面スクロール速度.db現在の譜面スクロール速度.Drums + 1.0)) / 502.8594);
                    }
                    else
                    {
                        nノート座標 = 0;
        ...
        private void t進行描画_リアルタイム判定数表示()
        {
            ...
            if (TJAPlayer3.ConfigIni.nPlayerCount == 1)
            {
                ...
                if (TJAPlayer3.DTX.nノーツ数[0] == 0 && TJAPlayer3.DTX.nノーツ数[1] == 0 && TJAPlayer3.DTX.nノーツ数[2] == 0)
                {
                    TJAPlayer3.Tx.ACHIEVEMENT?.t2D描画(TJAPlayer3.app.Device, 329, 128);
                }
                ...
                double dbPERFECT率 = ((100.0 * TJAPlayer3.stage演奏ドラム画面.nヒット数_Auto含まない.Drums.Perfect) / (double)nNowTotal);
                double dbGREAT率 = ((100.0 * TJAPlayer3.stage演奏ドラム画面.nヒット数_Auto含まない.Drums.Great / (double)nNowTotal));
                double dbMISS率 = ((100.0 * TJAPlayer3.stage演奏ドラム画面.nヒット数_Auto含まない.Drums.Miss / (double)nNowTotal));
                ...
                if (double.IsNaN(dbMISS率))
                    dbMISS率 = 0;
                this.t大文字表示(566, 158, string.Format("{0,6:f}%", dbたたけた率));
        ...
        private void t小文字表示( int x, int y, string str, bool bOrange )
		{
			foreach( char ch in str )
			{
				for( int i = 0; i < this.st小文字位置.Length; i++ )
				{
                    ...
                    if (TJAPlayer3.ConfigIni.bJudgeCountDisplay == true)
                    {
				...
				for( int i = 0; i < this.st小文字位置.Length; i++ )
				{
                    ...
                    if (TJAPlayer3.DTX.nノーツ数[0] == 0 && TJAPlayer3.DTX.nノーツ数[1] == 0 && TJAPlayer3.DTX.nノーツ数[2] == 0)
                    {
                        if (this.st小文字位置[i].ch == ch)
                        {
                            ...
                            var yScalling = TJAPlayer3.stage演奏ドラム画面.actCombo.ComboScale_Ex[TJAPlayer3.stage演奏ドラム画面.actCombo.ctコンボ加算[0].n現在の値, 0];
                            var yJumping = TJAPlayer3.stage演奏ドラム画面.actScore.ScoreScale[TJAPlayer3.stage演奏ドラム画面.actScore.ct点数アニメタイマ[0].n現在の値];

                            if (dbたたけた率 >= 97)
                            {
                                TJAPlayer3.Tx.Result_Number.vc拡大縮小倍率.Y = 1 + yScalling;
                                TJAPlayer3.Tx.Result_Number.t2D拡大率考慮下中心基準描画(TJAPlayer3.app.Device, x, y, rectangle);
                                TJAPlayer3.Tx.Result_Number_Period.t2D描画(TJAPlayer3.app.Device, 643, 144);
                                TJAPlayer3.Tx.Result_Number_Percent.t2D描画(TJAPlayer3.app.Device, 722, 127);
                            }
                            else if (dbたたけた率 >= 80)
                            {
                                TJAPlayer3.Tx.Result_Number_Red.vc拡大縮小倍率.Y = 1 + yScalling;
                                TJAPlayer3.Tx.Result_Number_Red.t2D拡大率考慮下中心基準描画(TJAPlayer3.app.Device, x, y, rectangle);
                                TJAPlayer3.Tx.Result_Number_Period_Red.t2D描画(TJAPlayer3.app.Device, 643, 144);
                                TJAPlayer3.Tx.Result_Number_Percent_Red.t2D描画(TJAPlayer3.app.Device, 722, 127);
                            }
                            else
                            {
                                TJAPlayer3.Tx.Result_Number_Normal.vc拡大縮小倍率.Y = 1 + yScalling;
                                TJAPlayer3.Tx.Result_Number_Normal.t2D拡大率考慮下中心基準描画(TJAPlayer3.app.Device, x, y, rectangle);
                                TJAPlayer3.Tx.Result_Number_Period_Normal.t2D描画(TJAPlayer3.app.Device, 643, 144);
                                TJAPlayer3.Tx.Result_Number_Percent_Normal.t2D描画(TJAPlayer3.app.Device, 722, 127);
                            }
