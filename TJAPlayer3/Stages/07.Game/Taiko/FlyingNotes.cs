...
namespace TJAPlayer3
{
    internal class FlyingNotes : CActivity
    {
                ...
                for (int i = 0; i < 1; i++)
                {
                    for (int j = 0; j < 64; j++)
                    {
                        if (!this.st飛び散るチップ[j].b使用中)
                        {
                            ...
                            this.st飛び散るチップ[j].ct進行 = new CCounter(0, 323 / 4, 7, TJAPlayer3.Timer); // カウンタ
                            ...
                            this.st飛び散るチップ[j].f半径 = (float)(0.6 + (((double)TJAPlayer3.Random.Next(30)) / 100.0));
                            break;
                        }
                        ...
                        //    this.st飛んで行く音符[j].ct進行 = new CCounter(0, 49, 12, TJAPlayer3.Timer); // カウンタ

                        //    this.st飛んで行く音符[j].fXL = TJAPlayer3.Skin.nScrollFieldX; //X座標
                        //    this.st飛んで行く音符[j].fY = 257 - 65;

                        //    int n頂点 = -18;
                        //    this.st飛んで行く音符[j].f加速度X = (float)( ( 1224 - TJAPlayer3.Skin.nScrollFieldX ) / 50.0f );
            ...
            if (TJAPlayer3.Tx.Effects_Rainbow != null)
            {
                for (int i = 0; i < 2; i++)
                {
                    if (!this.st虹[i].b使用中 && player == 0)
                    {
                        ...
                        this.st虹[i].ct進行 = new CCounter(0, 164, 5, TJAPlayer3.Timer); // カウンタ
                    ...
                    if (!this.st虹2[i].b使用中 && player == 1)
                    {
                        ...
                        this.st虹2[i].ct進行 = new CCounter(0, 164, 5, TJAPlayer3.Timer); // カウンタ
        ...
        public override void OnManagedリソースの作成()
        {
            if (!base.b活性化してない)
            {
                //this.tx音符 = TJAPlayer3.tテクスチャの生成( CSkin.Path( @"Graphics\7_taiko_notes.png" ) );
                //this.tx虹 = TJAPlayer3.tテクスチャの生成( CSkin.Path( @"Graphics\7_balloon_rainbow.png" ) );
        ...
        public override void OnManagedリソースの解放()
        {
            if (!base.b活性化してない)
            {
                //TJAPlayer3.tテクスチャの解放( ref this.tx音符 );
                //            TJAPlayer3.tテクスチャの解放( ref this.tx虹 );
            ...
            if (!base.b活性化してない)
            {
                for (int f = 0; f < 2; f++)
                {
                    if (this.st虹[f].b使用中)
                    {
                        ...
                        if (TJAPlayer3.Tx.Effects_Rainbow != null && this.st虹[f].nPlayer == 0) //画像が出来るまで
                        {
                ...
                for (int f = 0; f < 2; f++)
                {
                    if (this.st虹2[f].b使用中)
                    {
                        ...
                        if (TJAPlayer3.Tx.Effects_Rainbow != null && this.st虹2[f].nPlayer == 1) //画像が出来るまで
                        {
                            //this.st虹[f].ct進行.n現在の値 = 164;
                ...
                for (int i = 0; i < 64; i++)
                {
                    if (TJAPlayer3.Skin.nScrollFieldX[0] > 414 + 4)
                        break;
                    if (TJAPlayer3.Skin.nScrollFieldX[0] < 414 - 4)
                        break;
                    ...
                    if (this.st飛び散るチップ[i].b使用中)
                    {
                        ...
                        for (int n = this.st飛び散るチップ[i].n前回のValue; n < this.st飛び散るチップ[i].ct進行.n現在の値; n++)
                        {
                            ...
                            {
                                this.st飛び散るチップ[i].fXL += (float)((this.st飛び散るチップ[i].f加速度X * Math.Cos((140.0 * Math.PI / 180.0))) * 5);
                                this.st飛び散るチップ[i].fXR += (float)((this.st飛び散るチップ[i].f加速度X * Math.Cos((60.0 * Math.PI / 180.0))) * 5);

                                this.st飛び散るチップ[i].fY += (float)((this.st飛び散るチップ[i].f加速度Y * Math.Sin((60.0 * Math.PI / 180.0))) * 9.50f - Math.Exp(this.st飛び散るチップ[i].f重力加速度 * 2.0f) / 2.0f);
                        ...
                        if (TJAPlayer3.Tx.Notes != null)
                        {
                            ...
                            {
                                TJAPlayer3.Tx.Notes.t3D描画(TJAPlayer3.app.Device, mat2, new Rectangle((this.st飛び散るチップ[i].nLane + 3) * 130, 0, 130, 130));
                            }
                            else
                            {
                                TJAPlayer3.Tx.Notes.t3D描画(TJAPlayer3.app.Device, mat2, new Rectangle(this.st飛び散るチップ[i].nLane * 130, 0, 130, 130));
                            }
                        }

                        if (this.st飛び散るチップ[i].ct進行.b終了値に達した)
                        {
                            TJAPlayer3.stage演奏ドラム画面.actGauge.Start(this.st飛び散るチップ[i].nLane, E判定.Perfect, this.st飛び散るチップ[i].nPlayer);
                            TJAPlayer3.stage演奏ドラム画面.actChipEffects.Start(st飛び散るチップ[i].nPlayer, st飛び散るチップ[i].nLane);
                        }
                    }
                    ...
                    if (this.st飛んで行く音符[i].b使用中)
                    {
                        ...
                        if (TJAPlayer3.Tx.Notes != null)
                        {
                            //this.tx音符.t3D描画(TJAPlayer3.app.Device, mat, new Rectangle( this.st飛んで行く音符[i].nLane * 130, 0, 130, 130));
                        }

                        //if( this.st飛んで行く音符[ i ].ct進行.b終了値に達した )
                        {
                            //TJAPlayer3.stage演奏ドラム画面.actGauge.Start( this.st飛んで行く音符[i].nLane, E判定.Perfect );
                        }
                    }
                }
            }
