                ...
                if (pChip.nチャンネル番号 < 0x15 || (pChip.nチャンネル番号 >= 0x1A))
                {
                    if (!stRunners[Index].b使用中)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                stRunners[Index].nType = random.Next(1, 1 + 1);
                            }
                        }
                        if (TJAPlayer3.stage選曲.n確定された曲の難易度 != (int)Difficulty.Dan)
                        {
                        ...
                        {
                            stRunners[Index].ct進行 = new CCounter(0, 1280, 16, TJAPlayer3.Timer);
                            stRunners[Index].nOldValue = 0;
                            stRunners[Index].nNowPtn = 0;
                            stRunners[Index].fX = 0;
                        }
        ...
        public override int On進行描画()
        {
            for (int i = 0; i < 128; i++)
            {
                if (stRunners[i].b使用中)
                {
                    ...
                    {
                        if (TJAPlayer3.Tx.Runner_Dan != null)
                        {
                             TJAPlayer3.Tx.Runner_Dan.t2D描画(TJAPlayer3.app.Device, (int)(StartPoint_X[0] + stRunners[i].fX), 0, new Rectangle(stRunners[i].nNowPtn * 150, stRunners[i].nType * 184, 150, 184));
                        }
                    }
                }
            }
