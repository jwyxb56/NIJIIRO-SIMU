    ...
    internal class CActResultSongBar : CActivity
    {
        ...
        public override void OnManagedリソースの解放()
        {
            if (!base.b活性化してない)
            {
                ...
                TJAPlayer3.t安全にDisposeする(ref this.ct登場用);
                TJAPlayer3.t安全にDisposeする(ref this.ct登場時透明度);
        ...
        public override int On進行描画()
        {
            ...
            {
                if (TJAPlayer3.stage結果.actParameterPanel.ct全体待機.n現在の値 >= 4900)
                {
                    if (ct登場時透明度.b停止中) ct登場時透明度 = new CCounter(0, 255, 2, TJAPlayer3.Timer);
                    this.ct登場時透明度?.t進行();
                }
            }
            else
            {
                if (TJAPlayer3.stage結果.actParameterPanel.ct全体待機.n現在の値 >= 3900)
                {
                    if (ct登場時透明度.b停止中) ct登場時透明度 = new CCounter(0, 255, 2, TJAPlayer3.Timer);
                    this.ct登場時透明度?.t進行();
                }
            ...
            {
                this.txMusicName.n透明度 = ct登場時透明度.n現在の値;
                ...
                if (TJAPlayer3.stage選曲.n確定された曲の難易度 != (int)Difficulty.Dan)
                {
                    this.txStageText.n透明度 = ct登場時透明度.n現在の値;
